...
angular
    ...
    function ($routeProvider, $locationProvider, $translateProvider) {
      ...
      $routeProvider
        ...
        .when("/dashboard", {
          ...
          title: "Dashboard - GitAnonymizer",
        })
        .when("/pr-dashboard", {
          redirectTo: "/dashboard?tab=pull_requests",
        })
        .when("/anonymize/:repoId?", {
          templateUrl: "/partials/anonymize.htm",
          controller: "anonymizeController",
          title: "Anonymize - GitAnonymizer",
        })
        ...
        .when("/status/:repoId", {
          ...
          title: "Repository status - GitAnonymizer",
        })
        .when("/faq", {
          templateUrl: "/partials/faq.htm",
          controller: "faqController",
          title: "About - GitAnonymizer",
        })
        .when("/about", {
          templateUrl: "/partials/faq.htm",
          controller: "faqController",
        ...
        .when("/profile", {
          ...
          title: "Profile - GitAnonymizer",
        })
        .when("/pr/:pullRequestId", {
          templateUrl: "/partials/pullRequest.htm",
          controller: "pullRequestController",
          title: "Anonymous Pull Request - GitAnonymizer",
          reloadOnUrl: false,
        })
        .when("/r/:repoId/:path*?", {
          templateUrl: "/partials/explorer.htm",
          controller: "exploreController",
          title: "Anonymous Repository - GitAnonymizer",
          reloadOnUrl: false,
        })
  ...
  .controller("dashboardController", [
    ...
    function ($scope, $http, $location) {
      ...
      const dashboardTab = $location.search().tab;
      $scope.activeTab = dashboardTab === "pull_requests" ? "pull_requests" : "repositories";
      $scope.setActiveTab = (tab) => {
        $scope.activeTab = tab;
        if (tab === "pull_requests") {
          $location.search("tab", "pull_requests");
        } else {
      ...
      $scope.repoOrderBy = "-anonymizeDate";
      $scope.prOrderBy = "-anonymizeDate";
      ...
      function normalizeRepoRows(rows) {
        for (let repo of rows) {
          if (!repo.pageView) {
            repo.pageView = 0;
          }
          if (!repo.lastView) {
      ...
      function normalizePullRequestRows(rows) {
      ...
      function getRepositories() {
        $http.get("/api/user/anonymized_repositories").then(
          (res) => {
            ...
            normalizeRepoRows($scope.repositories);
            refreshTooltips();
          },
          (err) => {
            console.error(err);
      ...
      function getPullRequests() {
        $http.get("/api/user/anonymized_pull_requests").then(
          (res) => {
            ...
            normalizePullRequestRows($scope.pullRequests);
            refreshTooltips();
          },
          (err) => {
            console.error(err);
      ...
      $scope.copyMirrorLink = (repoId) => {
        const link = `${window.location.origin}/r/${repoId}/`;
        vmCopyText(link).then(
          () => {
            $scope.toasts.push({
              title: "Anonymous link copied",
              date: new Date(),
              body: link,
            });
            $scope.$applyAsync();
      ...
      $scope.repoFiler = (repo) => {
        if ($scope.repoFilters.status[repo.status] == false) return false;

        if ($scope.repoSearch.trim().length == 0) return true;

        if (repo.source.fullName.indexOf($scope.repoSearch) > -1) return true;
        if (repo.repoId.indexOf($scope.repoSearch) > -1) return true;
      ...
      $scope.copyPullRequestLink = (pullRequestId) => {
        const link = `${window.location.origin}/pr/${pullRequestId}/`;
        vmCopyText(link).then(
          () => {
            $scope.toasts.push({
      ...
      $scope.pullRequestFilter = (pr) => {
        if ($scope.prFilters.status[pr.status] == false) return false;

        if ($scope.prSearch.trim().length == 0) return true;

        if ((pr.source.pullRequestId + "").indexOf($scope.prSearch) > -1)
          return true;
        if (pr.source.repositoryFullName.indexOf($scope.prSearch) > -1)
          return true;
        if (pr.pullRequestId.indexOf($scope.prSearch) > -1) return true;
    ...
    function ($scope, $http, $sce, $routeParams, $location, $translate) {
      ...
      async function getDetails() {
        ...
        try {
          ...
          if (!$scope.manualRepoId || !$scope.repoId) {
  ...
  .controller("exploreController", [
    ...
    function ($scope, $http, $location, $routeParams, $sce, PDFViewerService) {
      ...
      $scope.copyCurrentLink = () => {
        const suffix = $scope.filePath ? "/" + $scope.filePath : "/";
        const link = `${window.location.origin}/r/${$scope.repoId}${suffix}`;
        vmCopyText(link).then(
          () => {
            $scope.toasts.push({
              title: "Link copied",
  ...
  .controller("anonymizePullRequestController", [
    ...
    function ($scope, $http, $sce, $routeParams, $location, $translate) {
      ...
      $scope.isUpdate = false;
      $scope.manualPullRequestId = false;
      ...
      function generatePullRequestMirrorId() {
        const o = parseGithubUrl($scope.pullRequestUrl);
        if ($scope.options.origin) {
          return o.repo + "-" + generateRandomId(4);
        }
      ...
      async function getDetails() {
        ...
        try {
          ...
          $scope.details = res.data;
          if (!$scope.manualPullRequestId || !$scope.pullRequestId) {
            $scope.pullRequestId = generatePullRequestMirrorId();
          }
        } catch (error) {
          if (error.data) {
            $translate("ERRORS." + error.data.error).then((translation) => {
      ...
      function displayErrorMessage(message) {
        switch (message) {
          case "pullRequestId_already_used":
            $scope.anonymizeForm.pullRequestId.$setValidity("used", false);
            break;
          case "invalid_pullRequestId":
            $scope.anonymizeForm.pullRequestId.$setValidity("format", false);
            break;
          case "options_not_provided":
            $scope.anonymizeForm.pullRequestId.$setValidity("format", false);
            break;
          case "pull_request_already_anonymized":
            $scope.anonymizeForm.pullRequestUrl.$setValidity("used", false);
            break;
          case "invalid_terms_format":
          ...
          case "repo_not_found":
            ...
            break;
          case "repo_not_accessible":
            $scope.anonymizeForm.pullRequestUrl.$setValidity("access", false);
            break;
          default:
      ...
      $scope.$watch("options.origin", () => {
        if ($scope.isUpdate || !$scope.pullRequestUrl) {
          return;
        }
        if ($scope.manualPullRequestId && $scope.pullRequestId) {
          return;
