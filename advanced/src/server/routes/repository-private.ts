...
function sanitizeRepoIdPrefix(input: string): string {
...
async function generateRepoId(base: string): Promise<string> {
  const prefix = sanitizeRepoIdPrefix(base);
  for (let i = 0; i < 50; i++) {
    const candidate = `${prefix}-${randomSuffix(5)}`;
    const existing = await AnonymizedRepositoryModel.findOne({
      repoId: candidate,
    });
    if (!existing) {
      return candidate;
...
function validateNewRepo(repoUpdate: any): void {
  ...
  if (!repoUpdate.source || !repoUpdate.source.branch) {
...
router.post(
  ...
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      repoUpdate.repoId = repo.model.repoId;
...
router.post("/", async (req: express.Request, res: express.Response) => {
  ...
  try {
    if (!repoUpdate.repoId) {
      const parsed = gh(repoUpdate.fullName || repoUpdate.repository || "");
      repoUpdate.repoId = await generateRepoId(parsed?.name || "repo");
    } else {
      const existing = await AnonymizedRepositoryModel.findOne({
        repoId: repoUpdate.repoId,
      });
      if (existing) {
        throw new AnonymousError("repoId_already_used", {
          httpStatus: 400,
