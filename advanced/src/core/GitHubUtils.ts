...
export async function getToken(repository: Repository) {
  const span = trace.getTracer("veilmirror-core").startSpan("GHUtils.getToken");
  ...
  try {
    ...
    if (ownerAccessToken) {
      ...
      const oauthClient = repository.owner.model.oauthApps?.github;
      const refreshClientId = oauthClient?.clientId || config.CLIENT_ID;
      const refreshClientSecret =
        oauthClient?.clientSecret || config.CLIENT_SECRET;
      ...
      if (
        ...
        if (refreshClientId && refreshClientSecret) {
          const url = `https://api.github.com/applications/${refreshClientId}/token`;
          const headers = {
          ...
          const res = await fetch(url, {
            ...
            headers: {
              ...
              Authorization:
                "Basic " +
                Buffer.from(
                  refreshClientId + ":" + refreshClientSecret
                ).toString("base64"),
            },
          });
          ...
          if (res.ok) {
            ...
            console.error("Unable to refresh token", res.status, reason);
          }
        }
      }
