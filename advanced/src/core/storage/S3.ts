...
export default class S3Storage extends StorageBase {
  ...
  async exists(repoId: string, path: string = ""): Promise<FILE_TYPE> {
    const span = trace.getTracer("veilmirror-core").startSpan("s3.exists");
  ...
  async rm(repoId: string, dir: string = ""): Promise<void> {
    const span = trace.getTracer("veilmirror-core").startSpan("s3.rm");
  ...
  async send(repoId: string, path: string, res: Response) {
    const span = trace.getTracer("veilmirror-core").startSpan("s3.send");
    span.setAttribute("repoId", repoId);
    span.setAttribute("path", path);
    try {
      if (!config.S3_BUCKET) throw new Error("S3_BUCKET not set");
  ...
  async fileInfo(repoId: string, path: string) {
    const span = trace.getTracer("veilmirror-core").startSpan("s3.fileInfo");
    span.setAttribute("repoId", repoId);
    span.setAttribute("path", path);
    try {
      if (!config.S3_BUCKET) throw new Error("S3_BUCKET not set");
  ...
  async read(repoId: string, path: string): Promise<Readable> {
    const span = trace.getTracer("veilmirror-core").startSpan("s3.rreadm");
    span.setAttribute("repoId", repoId);
    span.setAttribute("path", path);
    try {
      if (!config.S3_BUCKET) throw new Error("S3_BUCKET not set");
  ...
  async listFiles(repoId: string, dir: string = ""): Promise<IFile[]> {
    const span = trace.getTracer("veilmirror-core").startSpan("s3.listFiles");
    span.setAttribute("path", dir);
    try {
      if (!config.S3_BUCKET) throw new Error("S3_BUCKET not set");
  ...
  async extractZip(
    ...
    const span = trace.getTracer("veilmirror-core").startSpan("s3.extractZip");
    span.setAttribute("path", path);
  ...
  async archive(
    ...
    const span = trace.getTracer("veilmirror-core").startSpan("s3.archive");
    span.setAttribute("repoId", repoId);
    span.setAttribute("path", dir);
    try {
      if (!config.S3_BUCKET) throw new Error("S3_BUCKET not set");
