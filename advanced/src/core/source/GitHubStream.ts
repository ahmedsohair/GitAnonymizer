...
export default class GitHubStream extends GitHubBase {
  ...
  downloadFile(token: string, sha: string) {
    const span = trace.getTracer("veilmirror-core").startSpan("GHStream.downloadFile");
  ...
  async getFiles(progress?: (status: string) => void) {
    const span = trace.getTracer("veilmirror-core").startSpan("GHStream.getFiles");
  ...
  private async getGHTree(
    ...
    const span = trace.getTracer("veilmirror-core").startSpan("GHStream.getGHTree");
    span.setAttribute("sha", sha);
    try {
  ...
  private tree2Tree(
    ...
    const span = trace.getTracer("veilmirror-core").startSpan("GHStream.tree2Tree");
    span.setAttribute("parentPath", parentPath);
    try {
