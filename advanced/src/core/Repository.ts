...
export default class Repository {
  ...
  async files(
    ...
    const span = trace.getTracer("veilmirror-core").startSpan("Repository.files");
  ...
  async anonymize(progress?: (status: string) => void) {
    const span = trace.getTracer("veilmirror-core").startSpan("Repository.anonymize");
    span.setAttribute("repoId", this.repoId);
   ...
   */
  async countView() {
    const span = trace.getTracer("veilmirror-core").startSpan("Repository.countView");
    span.setAttribute("repoId", this.repoId);
    try {
   ...
   */
  async expire() {
    const span = trace.getTracer("veilmirror-core").startSpan("Repository.expire");
    span.setAttribute("repoId", this.repoId);
   ...
   */
  async remove() {
    const span = trace.getTracer("veilmirror-core").startSpan("Repository.remove");
    span.setAttribute("repoId", this.repoId);
  ...
  async resetSate(status?: RepositoryStatus, statusMessage?: string) {
    const span = trace.getTracer("veilmirror-core").startSpan("Repository.resetState");
    span.setAttribute("repoId", this.repoId);
    // remove attribute
    this._model.size = { storage: 0, file: 0 };
    if (status) {
  ...
  async conference(): Promise<Conference | null> {
    const span = trace.getTracer("veilmirror-core").startSpan("Repository.conference");
    span.setAttribute("repoId", this.repoId);
    try {
